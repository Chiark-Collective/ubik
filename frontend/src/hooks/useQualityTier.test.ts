...
describe("detectQualityTier", () => {
  describe("integrated graphics detection (low tier)", () => {
    ...
    it("should detect Mesa drivers as low", () => {
      expect(detectQualityTier("Mesa DRI Intel(R) HD Graphics 530")).toBe(
        "low",
      );
    });
...
describe("TIER_CONFIGS", () => {
  ...
  it("should have increasing particle counts from low to high", () => {
    expect(TIER_CONFIGS.low.maxParticles).toBeLessThan(
      TIER_CONFIGS.medium.maxParticles,
    );
    expect(TIER_CONFIGS.medium.maxParticles).toBeLessThan(
      TIER_CONFIGS.high.maxParticles,
    );
  });

  it("should have increasing emit rates from low to high", () => {
    expect(TIER_CONFIGS.low.emitRate).toBeLessThan(
      TIER_CONFIGS.medium.emitRate,
    );
    expect(TIER_CONFIGS.medium.emitRate).toBeLessThan(
      TIER_CONFIGS.high.emitRate,
    );
  });
});
