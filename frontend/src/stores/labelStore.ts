...
export interface SamplePointConstraint extends BaseConstraint {
  type: "sample_point";
  position: [number, number, number];
  distance: number; // Signed distance to surface (negative=solid, positive=empty)
}
  ...
  undo: (projectId) => {
    ...
    set((state) => {
      ...
      switch (action.type) {
        case "add":
          // Undo add = remove
          newConstraints = existing.filter(
            (c) => c.id !== action.constraint.id,
          );
          break;
        case "remove":
