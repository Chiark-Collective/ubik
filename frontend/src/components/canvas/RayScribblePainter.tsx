...
import {
  useLabelStore,
  type RayCarveConstraint,
} from "../../stores/labelStore";
import {
  useRayScribbleStore,
  type RayInfo as StoreRayInfo,
} from "../../stores/rayScribbleStore";
...
export function RayScribblePainter({ projectId }: RayScribblePainterProps) {
  ...
  const constraints = useLabelStore(
    (s) => s.constraintsByProject[projectId] ?? [],
  );
      ...
      if (raycastReady) {
        ...
        if (hit) {
          const hitPoint = raycaster.ray.origin
            .clone()
            .add(raycaster.ray.direction.clone().multiplyScalar(hit.distance));
          currentHitPointRef.current = hitPoint;
        } else {
          // No hit - aim at a point far away
          currentHitPointRef.current = raycaster.ray.origin
            .clone()
  ...
  const geometry = useMemo(() => {
    ...
    const rayData = rays.map((ray) => {
      ...
      const hitPoint = origin
        .clone()
        .add(direction.clone().multiplyScalar(ray.hitDistance));
      const bufferZone =
    ...
    rayData.forEach((ray, rayIndex) => {
      ...
      for (let j = 0; j < rayData.length; j++) {
        ...
        if (dirDot > 0.95) {
          // Similar direction (within ~18 degrees)
      ...
      const perpUp = new THREE.Vector3()
