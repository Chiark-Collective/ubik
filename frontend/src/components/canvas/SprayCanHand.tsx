...
export function SprayCanHand({
  ...
  const geometries = useMemo(() => ({
    // Spray can parts
    canBody: new THREE.CylinderGeometry(CAN.bodyRadius, CAN.bodyRadius, CAN.bodyHeight, segments),
    canCap: new THREE.CylinderGeometry(CAN.capRadius, CAN.capRadius, CAN.capHeight, segments),
    nozzle: new THREE.CylinderGeometry(CAN.nozzleRadius, CAN.nozzleRadius * 0.7, CAN.nozzleHeight, segments),

    // Hand parts
    palm: new THREE.BoxGeometry(HAND.palmWidth, HAND.palmHeight, HAND.palmDepth),
    finger: new THREE.CylinderGeometry(HAND.fingerRadius, HAND.fingerRadius * 0.85, HAND.fingerLength, segments),
    thumb: new THREE.CylinderGeometry(HAND.thumbRadius, HAND.thumbRadius * 0.85, HAND.thumbLength, segments),
  }), [segments])

  // Materials (memoized)
  const materials = useMemo(() => ({
    can: new THREE.MeshStandardMaterial({ color: COLORS.can, metalness: 0.6, roughness: 0.4 }),
    cap: new THREE.MeshStandardMaterial({ color: COLORS.canCap, metalness: 0.3, roughness: 0.6 }),
    nozzle: new THREE.MeshStandardMaterial({ color: COLORS.nozzle, metalness: 0.5, roughness: 0.5 }),
    skin: new THREE.MeshStandardMaterial({ color: COLORS.skin, metalness: 0.1, roughness: 0.8 }),
  }), [])
  ...
  useFrame((_, delta) => {
    ...
    triggerPressRef.current += (targetPress - triggerPressRef.current) * Math.min(1, delta * 12)
    ...
    <group ref={groupRef}>
      ...
      <group position={[0, 0, 0]} rotation={[0.1, 0, 0]}>
        ...
        <mesh
          ...
          position={[0, CAN.bodyHeight / 2 + CAN.capHeight + CAN.nozzleHeight / 2, 0]}
        />
      </group>

      {/* Hand */}
      <group position={[CAN.bodyRadius * 1.5 * mirrorX, -CAN.bodyHeight * 0.3, 0]}>
        {/* Palm */}
        <mesh
        ...
        <group
          ref={indexFingerRef}
          position={[HAND.palmWidth * 0.1 * mirrorX, HAND.palmHeight * 0.35, HAND.palmDepth * 0.3]}
        >
          <mesh
        ...
        <mesh
          geometry={geometries.finger}
          material={materials.skin}
          position={[HAND.palmWidth * 0.35 * mirrorX, HAND.palmHeight * 0.4 + HAND.fingerLength / 2, 0]}
          rotation={[-0.2, 0, 0]}
        />
        ...
        <mesh
          geometry={geometries.finger}
          material={materials.skin}
          position={[HAND.palmWidth * 0.55 * mirrorX, HAND.palmHeight * 0.35 + HAND.fingerLength / 2, -HAND.palmDepth * 0.15]}
          rotation={[-0.25, 0, 0]}
        />
        ...
        <mesh
          geometry={geometries.finger}
          material={materials.skin}
          position={[HAND.palmWidth * 0.7 * mirrorX, HAND.palmHeight * 0.25 + HAND.fingerLength / 2, -HAND.palmDepth * 0.3]}
        ...
        <mesh
          ...
          material={materials.skin}
          position={[-HAND.palmWidth * 0.1 * mirrorX, -HAND.palmHeight * 0.1, HAND.palmDepth * 0.4]}
